...
// Set to true to use gl.LINE_STRIP (thin but clean), false for miter quads (thick but complex)
const USE_LINES = true;

// ---- Shaders ----
// Thick curve with miter joins: each vertex knows prev+next neighbours,
// shader bisects the two segment directions for a seamless join.
const VERT_CURVE = `
  attribute vec3 aPos, aPrev, aNext; attribute float aSide, aT;
  uniform mat4 uMVP; uniform vec2 uRes; uniform float uThick;
  varying float vT;
  void main() {
    vec4 cp   = uMVP*vec4(aPos, 1.);
    vec4 cPrv = uMVP*vec4(aPrev,1.);
    vec4 cNxt = uMVP*vec4(aNext,1.);
    vec2 pScreen = cp.xy/cp.w * uRes;
    vec2 prvScreen = cPrv.xy/cPrv.w * uRes;
    vec2 nxtScreen = cNxt.xy/cNxt.w * uRes;

    // Direction from prev→this and this→next (in screen space)
    vec2 d1 = normalize(pScreen - prvScreen);
    vec2 d2 = normalize(nxtScreen - pScreen);

    // Miter: bisect the two perpendiculars
    vec2 n1 = vec2(-d1.y, d1.x);
    vec2 n2 = vec2(-d2.y, d2.x);
    vec2 miter = normalize(n1 + n2);

    // Scale miter length so it meets both quads (capped to avoid extreme miters)
    float len = uThick / max(dot(miter, n1), 0.4);
    vec2 off = aSide * len * miter / uRes;
...
// Simple line shader — just pos + t, used with gl.LINE_STRIP
const VERT_LINE = `
  attribute vec3 aPos; attribute float aT;
  uniform mat4 uMVP; varying float vT;
  void main() { gl_Position=uMVP*vec4(aPos,1.); vT=aT; }`;
const FRAG_LINE = `
  precision mediump float; varying float vT;
  void main() {
    float t=pow(vT,1.5);
...
async function init() {
  ...
  const progLine  = program(gl, VERT_LINE,  FRAG_LINE);
  const progFlat  = program(gl, VERT_FLAT,  FRAG_FLAT);

  const cLoc = {
    aPos:  gl.getAttribLocation(progCurve,'aPos'),
    aPrev: gl.getAttribLocation(progCurve,'aPrev'),
  ...
  const lLoc = {
    ...
    aT:   gl.getAttribLocation(progLine,'aT'),
  ...
  // Buffers
  const buf = {
    pos:  gl.createBuffer(), prev: gl.createBuffer(),
    next: gl.createBuffer(), side: gl.createBuffer(),
    t:    gl.createBuffer(), idx:  gl.createBuffer(),
    axis: gl.createBuffer(), plane: gl.createBuffer(),
    linePts: gl.createBuffer(), lineT: gl.createBuffer(),
  };
  let idxCount = 0, lineCount = 0;
  ...
  function rebuild() {
    ...
    const N = pts.length;
    // Per-vertex layout: 2 vertices per point (left/right sides), N*2 vertices total
    // Each vertex needs pos, prev, next, side, t
    const pos  = new Float32Array(N*2*3);
    const prev = new Float32Array(N*2*3);
    const next = new Float32Array(N*2*3);
    const side = new Float32Array(N*2);
    const t    = new Float32Array(N*2);
    // (N-1) quads → 2 triangles each → 6 indices
    const idx  = new Uint16Array((N-1)*6);

    for (let i=0; i<N; i++) {
      const p    = pts[i];
      const pPrv = pts[Math.max(0, i-1)];      // clamp at start
      const pNxt = pts[Math.min(N-1, i+1)];    // clamp at end
      const ti   = i / (N-1);
      for (let s=0; s<2; s++) {   // s=0: left (+1), s=1: right (-1)
        const vi = i*2 + s;
        pos[ vi*3]   = p.x;    pos[ vi*3+1] = p.y;    pos[ vi*3+2] = p.z;
        prev[vi*3]   = pPrv.x; prev[vi*3+1] = pPrv.y; prev[vi*3+2] = pPrv.z;
        next[vi*3]   = pNxt.x; next[vi*3+1] = pNxt.y; next[vi*3+2] = pNxt.z;
        side[vi] = s === 0 ? 1 : -1;
        t[vi]    = ti;
      }
    }

    // Quads: for each consecutive point pair i, i+1 → vertices i*2, i*2+1, (i+1)*2, (i+1)*2+1
    for (let i=0; i<N-1; i++) {
      const b = i*2, j = i*6;
    ...
    // Separate flat arrays for LINE_STRIP — one entry per point, no side duplication
    const linePts = new Float32Array(N*3);
    const lineT   = new Float32Array(N);
    for (let i=0; i<N; i++) {
      linePts[i*3]=pts[i].x; linePts[i*3+1]=pts[i].y; linePts[i*3+2]=pts[i].z;
      lineT[i] = i/(N-1);
    }
    gl.bindBuffer(gl.ARRAY_BUFFER, buf.linePts); gl.bufferData(gl.ARRAY_BUFFER, linePts, u);
    gl.bindBuffer(gl.ARRAY_BUFFER, buf.lineT);   gl.bufferData(gl.ARRAY_BUFFER, lineT,   u);

    gl.bindBuffer(gl.ARRAY_BUFFER, buf.pos);  gl.bufferData(gl.ARRAY_BUFFER, pos,  u);
    gl.bindBuffer(gl.ARRAY_BUFFER, buf.prev); gl.bufferData(gl.ARRAY_BUFFER, prev, u);
  ...
  function render() {
    ...
    // Curve
    if (USE_LINES && lineCount > 1) {
      gl.useProgram(progLine); gl.uniformMatrix4fv(lLoc.uMVP,false,mvp);
      gl.bindBuffer(gl.ARRAY_BUFFER,buf.linePts); gl.enableVertexAttribArray(lLoc.aPos); gl.vertexAttribPointer(lLoc.aPos,3,gl.FLOAT,false,0,0);
      gl.bindBuffer(gl.ARRAY_BUFFER,buf.lineT);   gl.enableVertexAttribArray(lLoc.aT);   gl.vertexAttribPointer(lLoc.aT,  1,gl.FLOAT,false,0,0);
      gl.drawArrays(gl.LINE_STRIP, 0, lineCount);
    } else if (!USE_LINES && idxCount>0) {
      ...
      gl.bindBuffer(gl.ARRAY_BUFFER,buf.prev); gl.enableVertexAttribArray(cLoc.aPrev); gl.vertexAttribPointer(cLoc.aPrev,3,gl.FLOAT,false,0,0);
